...
/**
 * Transliterates rusian and ukrainian word as cyrillic to latin.
 * 
 * @param  {String} word
 * @return {String} transliterated word
 * @api public
 */
var transliterate = exports.transliterate = function(word) {
	if (!isString(str) || !str.length) return '';
	var rtn = [];
	// applying зг-zgh rule
	word = word.replace(transliteration.regexp.Zgh, 'Zgh');
	word = word.replace(transliteration.regexp.zgh, 'zgh');
	// replace characters with equivalent maps
	for (var i = 0; i < word.length; i++) {
		var character = word[i],
			latinCharacter = transliteration.mapping[character];
