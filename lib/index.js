...
var keyToLabel = exports.keyToLabel = function(str) {
  ...
  var latinFrom = "àáäâèéëêìíïîòóöôùúüûñç";
  var latinTo   = "aaaaeeeeiiiioooouuuunc";

	str = str.replace(/([a-z])([A-Z])/g, '$1 $2');
	str = str.replace(/([0-9])([a-zA-Z])/g, '$1 $2');
	str = str.replace(/([a-zA-Z])([0-9])/g, '$1 $2');

  for (var i=0, l=latinFrom.length ; i<l ; i++) {
    str = str.replace(new RegExp(latinFrom.charAt(i), 'g'), latinTo.charAt(i));
  }
